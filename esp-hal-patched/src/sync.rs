...
/// A simple critical-section-guarded cell, re-added for esp-wifi compatibility.
...
impl<T> Locked<T> {
    pub const fn new(val: T) -> Self {
        Locked(UnsafeCell::new(val))
    }

    pub fn with<F: FnOnce(&mut T) -> R, R>(&self, f: F) -> R {
        ::critical_section::with(|_cs| f(unsafe { &mut *self.0.get() }))
    }
}

#[cfg(riscv)]
use esp_sync::raw::SingleCoreInterruptLock;
pub use esp_sync::{RawMutex, RestoreState};
use esp_sync::{GenericRawMutex, raw::RawLock};
